...
export default function PatientDB({ onClose, newDbFlag }: Props) {
  ...
  const pid: string = params.id as string;

  useEffect(() => {
    function filterPatientsByPid(patients: Patient[], targetPid: string): Patient[] {
      return patients.filter((patient) => patient.pid === targetPid);
    }
    ...
    const getPatient = async () => {
      try {
        ...
        const filteredList = filterPatientsByPid(response.content, pid);
        setPatientList((prev) => [...prev, ...filteredList]);
      } catch (err: unknown) {
        console.log(err);
      }
    };
