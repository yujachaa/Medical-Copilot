...
export default function SelectTabIcons(logoType: PluginType) {
  if (logoType === 'default') return <Default className="w-8 h-8 ml-4" />;
  if (logoType === 'capsule') return <Capsule className="w-8 h-8 ml-4" />;
  if (logoType === 'cxr') return <Cxr className="w-8 h-8 ml-4" />;
}
